...
def xy_to_rphi(x, y):
    ...
    # x axis aligns with the center of scan, pointing outward/upward, y axis pointing to right
    # phi is the angle with x axis, rotating towards y is positive
...
def global_to_canonical_flow(flow, scan_phi):
    sin = np.sin(scan_phi)
    cos = np.cos(scan_phi)

    rot = np.append(
        np.expand_dims(np.vstack([cos, -sin]).T, axis=1),
        np.expand_dims(np.vstack([sin, cos]).T, axis=1),
        axis=1,
    )

    flow_canonical = np.einsum("ijk,ik->ij", rot, flow)
    # flow_canonical = canonical_to_global_flow(flow_canonical, scan_phi)

    return flow_canonical


def canonical_to_global_flow(flow_canonical, scan_phi):
    sin = np.sin(scan_phi)
    cos = np.cos(scan_phi)
    ...
    flow = np.einsum("ijk,ik->ij", rot, flow_canonical)
    return flow


def canonical_to_global_flow_torch(flow_canonical, scan_phi):
    sin = np.sin(scan_phi)
    cos = np.cos(scan_phi)
    ...
    rot = torch.from_numpy(rot).cuda(non_blocking=True).float()

    flow = torch.einsum("ijk,ik->ij", rot, flow_canonical)
    return flow
...
# def get_flow_target(scan, scan_phi, delta_odom, idx):
#     delta_phi = delta_odom[-1]
#     scan_xy = np.array(rphi_to_xy(scan, scan_phi)).T
#     flow = np.cross(np.array([0, 0, delta_phi]) , scan_xy)[:, :-1] - delta_odom[:2]
#
#     return flow

# def get_flow_target_canocial(scan, scan_phi, delta_odom, idx):
#     delta_phi = delta_odom[-1]
#     scan_xy = np.array(rphi_to_xy(scan, scan_phi)).T
#     flow = np.cross(np.array([0, 0, delta_phi]) , scan_xy)[:, :-1] - delta_odom[:2]
#     flow_canonical = global_to_canonical_flow(flow, scan_phi)
#
#     return flow_canonical


def get_flow_target(scan, scan_phi, odom_0, odom_1, to_canonical=False):

    # Rotation from world to frame 0
    sphi_0 = np.sin(odom_0[-1])
    cphi_0 = np.cos(odom_0[-1])
    rot_0 = np.array([[cphi_0, -sphi_0], [sphi_0, cphi_0]])

    # Rotation from scan in frame 0 to scan in frame 1
    delta_phi = odom_1[-1] - odom_0[-1]
    sphi_1 = np.sin(delta_phi)
    cphi_1 = np.cos(delta_phi)
    rot_1 = np.array([[cphi_1, -sphi_1], [sphi_1, cphi_1]])

    # Transform translation in world frame into frame of scan 0
    trans_world = odom_1[:2] - odom_0[:2]
    trans_frame_0 = np.matmul(trans_world, rot_0.T)

    scan_xy_frame_0 = np.array(rphi_to_xy(scan, scan_phi)).T
    scan_xy_frame_1 = np.matmul(scan_xy_frame_0, rot_1.T) - trans_frame_0

    flow = scan_xy_frame_1 - scan_xy_frame_0

    if to_canonical:
        flow = global_to_canonical_flow(flow, scan_phi)
...
def flow_to_hsv(flow):
    ...
    for idx in range(len(flow)):
        r, phi = xy_to_rphi(flow[idx, 0], flow[idx, 1])
        hsv = np.array([((phi + 2.0 * np.pi) / np.pi / 2), min(r, 0.1) / 0.1, 1.0])
...
def plot_histograms(data, title):
    ...
    N, bins, patches = ax.hist(
        data, bins=num_bins, weights=np.ones(len(data)) / len(data)
    )
...
def _phi_to_rotation_matrix(phi, is_3d=False):
    cp, sp = np.cos(phi), np.sin(phi)
    if is_3d:
        return np.array([[cp, -sp, 0], [sp, cp, 0], [0, 0, 1]], dtype=np.float32)
    else:
...
def get_displacement_from_odometry(scan1_xy, odom0, odom1):
    ...
    Args:
        scan1_xy (array[N, (x, y)]): Stationary points expressed in current scanner frame.
        odom0 (array[(x, y, phi)]): Sensor odometry at previous step (time0).
        odom1 (array[(x, y, phi)]): Sensor odometry at current step (time1).
